...
import {
	AuthMode,
	CredentialManager,
} from "../dist/auth/credential-manager.js";
...
async function testProfileManager() {
	...
	if (activeName !== "test-profile") {
		throw new Error(
			'Expected active profile "test-profile", got "' + activeName + '"',
		);
	}
	...
	if (deleteActiveResult.success) {
		throw new Error(
			"Should not be able to delete active profile without force",
		);
	}
...
async function testCredentialManagerWithProfiles() {
	...
	if (cm.getTenant() !== "cred-tenant") {
		throw new Error(
			"Tenant mismatch: expected cred-tenant, got " + cm.getTenant(),
		);
	}
