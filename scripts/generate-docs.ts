 ...
 */
function formatSideEffects(
	sideEffects: AggregatedMetadata["sideEffects"],
): string {
	const hasEffects =
...
function generateMarkdown(resourceDoc: ResourceDoc): string {
	...
	const rawDescription =
		summary || `Manage ${title} resources in F5 Distributed Cloud.`;
		...
		if (pathParams.size > 0) {
			...
			for (const [name, desc] of pathParams) {
				...
				const example = escapeTableCell(
					metadata.parameterExamples[name] || "-",
				);
...
`;
	...
	const bodyDescription = escapeJsx(
		sanitizeDescription(
			description ||
				summary ||
				`Manage ${title} resources in F5 Distributed Cloud.`,
		),
	);
...
function groupToolsByResource(
	...
	for (const tool of tools) {
		...
		if (
			tool.description &&
			tool.description.length > (doc.description?.length ?? 0)
		) {
...
async function generateDocs(): Promise<void> {
	...
	for (const [, resourceDoc] of resourceDocs) {
		...
		const outputDir = join(
			CONFIG.DOCS_DIR,
			resourceDoc.categoryPath.directoryPath,
		);
