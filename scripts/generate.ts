...
async function formatCode(code: string): Promise<string> {
	try {
		...
		log.warn(
			`Prettier formatting failed, using unformatted code: ${error instanceof Error ? error.message : String(error)}`,
		);
		return code;
	}
}
...
function generateDomainFile(
	...
	const toolDefs = operations.map((op) => {
		...
		const sortedRequiredParams = [...op.requiredParams].sort((a, b) =>
			a < b ? -1 : a > b ? 1 : 0,
		);
		const sortedTags = [...op.tags].sort((a, b) =>
			a < b ? -1 : a > b ? 1 : 0,
		);

		// Sort new rich metadata arrays for deterministic output
		const sortedRequiredFields = [...op.requiredFields].sort((a, b) =>
			a < b ? -1 : a > b ? 1 : 0,
		);
...
async function generate(): Promise<void> {
	...
	log.info(
		`Dependency graph: ${dependencyGraph.totalResources} resources mapped`,
	);
	...
	console.log(
		`  Dependency graph: ${dependencyGraph.totalResources} resources`,
	);
