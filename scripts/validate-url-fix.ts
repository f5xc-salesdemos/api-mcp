...
async function runValidation() {
  ...
  if (resourcePath) {
    ...
    logTest("Path normalization prevents /api/api", !hasDoubleApi, `Full URL: ${fullUrl}`);
  }
  ...
  try {
    ...
    logTest("Live API call succeeded", response.status === 200, `Status: ${response.status}, Has items: ${hasItems}`);
  } catch (error) {
  ...
  const testFormats = ["f5-amer-ent.console.ves.volterra.io", "f5-amer-ent.console.ves.volterra.io/api"];
