 ...
 */
async function extractDomainSpecs(zipPath: string, destDir: string): Promise<number> {
...
async function syncSpecs(): Promise<void> {
  ...
  try {
    ...
    const extractedCount = await extractDomainSpecs(CONFIG.TEMP_ZIP, CONFIG.SPECS_DOMAINS_DIR);
