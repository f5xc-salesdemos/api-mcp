...
async function main(): Promise<void> {
	...
	try {
		if (!existsSync(FIXTURES_FILE)) {
			addResult("Test Fixtures", {
				passed: false,
				message:
					"tests/fixtures/generated.ts not found - run npm run generate:fixtures",
			});
		} else {
	...
	try {
		...
		if (!allTools || !Array.isArray(allTools)) {
			...
			const coverage = {
				dangerLevel: allTools.filter(
					(t: ToolWithMetadata) => t.dangerLevel !== null,
				).length,
				sideEffects: allTools.filter(
					(t: ToolWithMetadata) => t.sideEffects !== null,
				).length,
				operationMetadata: allTools.filter(
					(t: ToolWithMetadata) => t.operationMetadata !== null,
				).length,
				parameterExamples: allTools.filter(
					(t: ToolWithMetadata) =>
						Object.keys(t.parameterExamples || {}).length > 0,
				).length,
			};

			const details = Object.entries(coverage).map(
				([field, count]) =>
					`${field}: ${count}/${total} (${((count / total) * 100).toFixed(1)}%)`,
			);

			// At least dangerLevel or operationMetadata should have good coverage (50%+)
			const hasGoodCoverage =
				coverage.dangerLevel > total * 0.5 ||
				coverage.operationMetadata > total * 0.5;

			addResult("Rich Metadata Coverage", {
				passed: hasGoodCoverage,
				message: hasGoodCoverage
					? "good coverage"
