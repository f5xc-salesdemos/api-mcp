...
const stats = {
	...
	domainStats: new Map<
		string,
		{ total: number; passed: number; failed: number; rbac: number }
	>(),
};
		...
		for (const domainName of domainDirs) {
			...
			if (fs.existsSync(indexPath)) {
				try {
					...
					const toolsArrayName = Object.keys(module).find((key) =>
						key.endsWith("Tools"),
					);
					...
					console.warn(
						`Warning: Could not load tools from ${domainName}:`,
						error,
					);
				}
			}
...
function shouldSkipDomain(domain: string): boolean {
	...
	return (
		failureRate > TEST_CONFIG.domainFailureThreshold && domainStat.total > 10
	);
}
...
function printFinalStatistics(): void {
	...
	for (const [domain, domainStat] of sortedDomains) {
		...
		const icon =
			domainStat.failed === 0
				? "✅"
				: domainStat.failed > domainStat.passed
					? "❌"
					: "⚠️";
...
async function validateCreateOperation(
	...
	try {
		const response = await executeWithRateLimit(() =>
			httpClient.post(testUrl, payload),
		);
 ...
 */
async function validateDocumentationMode(
	tool: any,
	domain: string,
): Promise<void> {
				...
				it(
					...
					async () => {
						...
						if (shouldSkipDomain(domain)) {
							console.log(
								`⏭️  Skipping ${domain} domain due to high failure rate`,
							);
							stats.skipped++;
							return;
						}
