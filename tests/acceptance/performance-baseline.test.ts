...
describe("Performance Baseline Tests", () => {
	describe("Response Time Benchmarks", () => {
		...
		it("should complete 10 sequential requests within 20 seconds", async () => {
			...
			console.log(
				`  ⏱️  Sequential 10x: ${duration}ms total, ${avgDuration.toFixed(0)}ms avg`,
			);
	...
	describe("Memory Usage Benchmarks", () => {
		it("should not leak memory over 100 requests (< 10MB growth)", async () => {
			...
			console.log(
				`  📈 Memory growth: ${growth.heapUsedGrowthMB.toFixed(2)} MB\n`,
			);

			// Allow some growth, but not excessive
			expect(Math.abs(growth.heapUsedGrowthMB)).toBeLessThan(
				THRESHOLDS.memoryGrowth,
			);
		});
		...
		it("should maintain stable memory with concurrent requests", async () => {
			...
			console.log(
				`  📈 Concurrent memory growth: ${growth.heapUsedGrowthMB.toFixed(2)} MB`,
			);
	...
	describe("Concurrency Benchmarks", () => {
		...
		it("should scale linearly with moderate concurrency", async () => {
			...
			console.log(
				`  📈 Scaling ratios: 2:1=${ratio_2_to_1.toFixed(2)}x, 10:5=${ratio_10_to_5.toFixed(2)}x\n`,
			);
