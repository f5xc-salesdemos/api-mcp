...
describe("URL Normalization Acceptance Tests", () => {
  describe("URL Format Variations", () => {
    ...
    it.each(urlVariations)("should normalize $description: $input -> $expected", ({ input, expected }) => {
      expect(normalizeF5XCUrl(input)).toBe(expected);
    });
  });

  describe("Path Normalization Prevents /api/api Duplication", () => {
    const resourceTypes = ["namespace", "certificate", "http_loadbalancer", "origin_pool", "dns_zone", "app_firewall"];

    it.each(resourceTypes)("should prevent /api/api for resource type: %s", (resourceType: string) => {
  ...
  describe("Configure Auth URL Normalization", () => {
    ...
    it("should normalize URL when configuring with protocol-less URL", async () => {
      ...
      expect((result as { message: string }).message).toContain("https://f5-amer-ent.console.ves.volterra.io");
    });
    ...
    it("should verify URL when skipVerification is false", async () => {
      ...
      expect(fetch).toHaveBeenCalledWith("https://f5-amer-ent.console.ves.volterra.io/api", expect.any(Object));
    });
  ...
  describe("Real-World URL Patterns from User Reports", () => {
    ...
    it.each(userReportedPatterns)("should handle $description without causing /api/api", ({ input }) => {
      // Normalize the URL
