...
export function predictOutcome(context: ScenarioContext): PredictedOutcome {
	...
	if (
		...
		const condition = findCondition(
			`${context.operation}_nonexistent_authenticated`,
		);
		if (condition) {
			return {
				condition,
...
export function validateHealthcheckConfig(
	...
	if (
		problemFields.some((f) =>
			HEALTHCHECK_ONEOF_GROUPS.hostHeaderChoice.some((g) => f.includes(g)),
		)
	) {
		inputState = "oneOf_conflict";
	} else if (
		problemFields.some((f) => f.includes("required") || f === "metadata.name")
	) {
		inputState = "missing_required";
	} else if (
		problemFields.some((f) => messages.some((m) => m.includes("Invalid type")))
	) {
...
export function areFieldsMutuallyExclusive(
	...
	for (const fields of Object.values(HEALTHCHECK_ONEOF_GROUPS)) {
		if (
			fields.includes(field1 as (typeof fields)[number]) &&
