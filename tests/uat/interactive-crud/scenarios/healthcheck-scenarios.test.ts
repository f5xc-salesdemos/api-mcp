...
describe("Healthcheck Interactive CRUD - English Prompt Simulation", () => {
		...
		it.each(
			...
			for (const [key, expectedValue] of Object.entries(
				...
				if (key === "name") {
					...
					expect(parsed.extractedValues.useOriginServerName).toBe(
						expectedValue,
					);
				}
			}
	...
	describe("Validation Prompts - OneOf Detection", () => {
		...
		it("should validate oneOf conflict in configuration", () => {
			...
			expect(
				validation.messages.some((m) => m.includes("mutually exclusive")),
			).toBe(true);
		});
		...
		it.each(
			getOneOfConflictPrompts(),
		)("should detect oneOf conflict in: '$prompt'", async (scenario) => {
			const parsed = parsePrompt(scenario.prompt);
			...
			const hasConflictingFields =
				spec.host_header !== undefined &&
				spec.use_origin_server_name !== undefined;
	...
	describe("Full Prompt-to-Tool Integration", () => {
		it("should process complete creation workflow", () => {
			// 1. Parse user prompt
			const prompt =
				"Create a healthcheck named api-monitor with HTTP path /health";
			const parsed = parsePrompt(prompt);
