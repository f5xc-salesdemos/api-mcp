...
export const INSPECTION_PATTERNS = {
  ...
  /** Get in namespace */
  getInNamespace: ["Show {resource} {name} in namespace {namespace}", "Get {resource} {name} from {namespace}"],
} as const;
...
export const MODIFICATION_PATTERNS = {
  ...
  multiFieldUpdate: ["Update {resource} {name}: set {field1} to {value1} and {field2} to {value2}"],
} as const;
...
export const DELETION_PATTERNS = {
  ...
  deleteInNamespace: ["Delete {resource} {name} from namespace {namespace}", "Remove {resource} {name} in {namespace}"],
} as const;
...
export const VALIDATION_PATTERNS = {
  ...
  /** Schema validation */
  validateSchema: ["Does this {resource} have all required fields?", "Validate {resource} schema"],
  /** OneOf validation */
  validateOneOf: [
...
export const RESOURCE_KEYWORDS: Record<string, string[]> = {
  ...
  "http-loadbalancer": ["http load balancer", "http-loadbalancer", "load balancer", "lb", "http lb"],
