...
const PLAIN_LANGUAGE_QUERIES = [
  ...
  },
  {
    ...
    minimalConfig: {
      ...
      spec: {
        origin_servers: [{ public_ip: { ip: "192.0.2.1" } }, { public_ip: { ip: "192.0.2.2" } }],
        port: 8080,
      },
    },
  },
...
const ORIGIN_POOL_VARIANTS = {
  ...
  "spec.loadbalancer_algorithm": ["ROUND_ROBIN", "LEAST_REQUEST", "RING_HASH", "RANDOM", "LB_OVERRIDE"],
...
describe("Origin Pool - Critical UI vs Server Discrepancy", () => {
  ...
  it("should verify server default for loadbalancer_algorithm", () => {
    const algorithmDefault = schemaMetadata.serverDefaults.find((d) => d.fieldPath.includes("loadbalancer_algorithm"));
  ...
  it("should verify server default for endpoint_selection", () => {
    const endpointDefault = schemaMetadata.serverDefaults.find((d) => d.fieldPath.includes("endpoint_selection"));
    ...
    describe(`Query: "${test.query}"`, () => {
      it("should find origin pool tool via search", () => {
        ...
        const hasOriginPool = results.some(
          (r) => r.tool.resource.includes("origin-pool") || r.tool.name.includes("origin-pool"),
        );
...
describe("Origin Pool - Server Defaults Documentation", () => {
  ...
  it("should verify minimal config uses server defaults", () => {
    ...
    expect(analysis.providedFields.some((f) => f.includes("loadbalancer_algorithm"))).toBe(false);
    expect(analysis.providedFields.some((f) => f.includes("endpoint_selection"))).toBe(false);
  });
});
