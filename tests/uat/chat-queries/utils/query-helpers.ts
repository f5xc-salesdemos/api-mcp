...
export function isDocumentationResponse(
	result: unknown,
): result is DocumentationResponse {
	return (
 ...
 */
export function isDependencyReport(
	result: unknown,
): result is DependencyReportResponse {
	return (
		typeof result === "object" &&
		result !== null &&
		"resource" in result &&
 ...
 */
export function isBestPracticesResponse(
	result: unknown,
): result is BestPracticesResponse {
	return (
		typeof result === "object" &&
		result !== null &&
		"success" in result &&
...
export function validateSearchResults(
	...
	if (topScore < expectedScore) {
		failures.push(
			`Top score ${topScore.toFixed(2)} below expected ${expectedScore}`,
		);
	}
