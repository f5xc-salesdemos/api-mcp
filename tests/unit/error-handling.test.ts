...
describe("error-handling", () => {
  ...
  describe("F5XCApiError", () => {
    it("should create an API error with status and response", () => {
      const error = new F5XCApiError("API failed", 500, {
  ...
  describe("ToolExecutionError", () => {
    ...
    it("should convert to JSON with tool name", () => {
      const error = new ToolExecutionError("f5xc-api-test", "Failed", {
        reason: "timeout",
      });
      const json = error.toJSON();
      expect(json).toEqual({
  ...
  describe("SpecificationError", () => {
    ...
    it("should support context", () => {
      const error = new SpecificationError("Parse error", {
        file: "spec.yaml",
      });
  ...
  describe("SSLCertificateError", () => {
    it("should create an SSL error with hostname and cert info", () => {
      const error = new SSLCertificateError("Certificate mismatch", "tenant.staging.console.ves.volterra.io", {
    ...
    it("should convert to JSON with hostname and cert info", () => {
      const error = new SSLCertificateError("Test", "example.com", { altNames: ["*.example.com"] }, { attempt: 1 });
      const json = error.toJSON();
      expect(json).toEqual({
  ...
  describe("wrapSSLError", () => {
    it("should wrap altname errors with staging guidance", () => {
      ...
      const wrapped = wrapSSLError(originalError, "https://tenant.staging.console.ves.volterra.io/api");
      ...
      expect((wrapped as SSLCertificateError).hostname).toBe("tenant.staging.console.ves.volterra.io");
    });

    it("should wrap altname errors without staging guidance for non-staging URLs", () => {
      const originalError = new Error("Hostname/IP does not match certificate's altnames");
      const wrapped = wrapSSLError(originalError, "https://tenant.console.ves.volterra.io/api");
