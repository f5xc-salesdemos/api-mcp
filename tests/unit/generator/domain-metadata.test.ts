...
describe("domain-metadata", () => {
	...
	describe("getDomainsByCategory", () => {
		it("should return domains for a valid category", () => {
			...
			expect(
				domains.every(
					(d) => d.domainCategory.toLowerCase() === category.toLowerCase(),
				),
			).toBe(true);
		});
	...
	describe("getAcronymsByCategory", () => {
		it("should filter acronyms by category", () => {
			...
			if (categories.length > 0) {
				...
				expect(
					filtered.every(
						(a) => a.category.toLowerCase() === categories[0].toLowerCase(),
					),
				).toBe(true);
			}
		});
	...
	describe("type safety", () => {
		it("should return correctly typed DomainMetadata", () => {
			const index = loadSpecIndex();
			const metadata: DomainMetadata | undefined = getDomainMetadata(
				index.specifications[0].domain,
			);
			...
			if (metadata) {
				...
				const _complexity: "simple" | "moderate" | "advanced" =
					metadata.complexity;
