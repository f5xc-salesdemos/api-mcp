...
describe("Resource Types", () => {
	...
	it("should have related resources defined", () => {
		expect(RESOURCE_TYPES.http_loadbalancer.relatedResources).toContain(
			"origin_pool",
		);
		expect(RESOURCE_TYPES.http_loadbalancer.relatedResources).toContain(
			"app_firewall",
		);
		expect(RESOURCE_TYPES.origin_pool.relatedResources).toContain(
			"healthcheck",
		);
	});
});
...
describe("buildResourceUri", () => {
	...
	it("should handle special characters in names", () => {
		...
		expect(uri).toBe(
			"f5xc://example-tenant/prod-ns/http_loadbalancer/app-lb-1",
		);
	});
});
...
describe("parseResourceUri", () => {
	it("should parse valid URI correctly", () => {
		const result = parseResourceUri(
			"f5xc://mytenant/production/http_loadbalancer/example-lb",
		);
	...
	it("should return null for invalid scheme", () => {
		const result = parseResourceUri(
			"https://mytenant/production/http_loadbalancer/example-lb",
		);
	...
	it("should return null for missing components", () => {
		expect(
			parseResourceUri("f5xc://mytenant/production/http_loadbalancer"),
		).toBeNull();
	...
	it("should return null for extra components", () => {
		const result = parseResourceUri(
			"f5xc://mytenant/production/http_loadbalancer/example-lb/extra",
		);
		expect(result).toBeNull();
	});
	...
	it("should return null for empty components", () => {
		expect(
			parseResourceUri("f5xc://mytenant//http_loadbalancer/example-lb"),
		).toBeNull();
		expect(
			parseResourceUri("f5xc:///production/http_loadbalancer/example-lb"),
		).toBeNull();
	});
});
...
describe("buildApiPath", () => {
	...
	it("should build path with resource name", () => {
		...
		expect(path).toBe(
			"/api/config/namespaces/production/http_loadbalancers/example-lb",
		);
	});
	...
	it("should handle different namespace values", () => {
		...
		expect(path2).toBe(
			"/api/config/namespaces/system/origin_pools/global-pool",
		);
	});
});
