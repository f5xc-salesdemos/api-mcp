...
describe("volterra-mapping", () => {
  describe("generateToolName", () => {
    it("should generate correct tool name format", () => {
      expect(generateToolName("waap", "http_loadbalancer", "create")).toBe("f5xc-api-waap-http-loadbalancer-create");
    });

    it("should normalize domain to lowercase alphanumeric", () => {
      expect(generateToolName("WAAP", "origin_pool", "list")).toBe("f5xc-api-waap-origin-pool-list");
    ...
    it("should convert underscores to hyphens in resource", () => {
      expect(generateToolName("core", "http_loadbalancer", "delete")).toBe("f5xc-api-core-http-loadbalancer-delete");
    });

    it("should handle special characters", () => {
      expect(generateToolName("net!work", "fire@wall", "update")).toBe("f5xc-api-network-firewall-update");
    });
  ...
  describe("extractResourceFromPath", () => {
    it("should extract resource from plural path", () => {
      ...
      expect(extractResourceFromPath("/api/config/namespaces/{namespace}/origin_pools")).toBe("origin-pool");
    });

    it("should extract resource from path with trailing parameter", () => {
      expect(extractResourceFromPath("/api/config/namespaces/{namespace}/http_loadbalancers/{name}")).toBe(
        "http-loadbalancer",
      );
    });
