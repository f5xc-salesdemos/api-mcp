...
const { mockConnect, mockClose, mockTool, mockResource, mockPrompt } =
	vi.hoisted(() => ({
...
const { mockReadResource, mockListResources, mockListResourceTemplates } =
	vi.hoisted(() => ({
	...
	describe("F5XCApiServer", () => {
		describe("constructor", () => {
			...
			it("should create HTTP client when authenticated", async () => {
				...
				expect(server.getCredentialManager().getAuthMode()).toBe(
					AuthMode.TOKEN,
				);
			});
		...
		describe("resource registration", () => {
			...
			it("should register namespace-scoped resources with correct URI template", async () => {
				...
				const namespacedResource = mockResource.mock.calls.find((call) =>
					call[0].includes("{namespace}"),
				);
			...
			it("should register system resources with correct URI template", async () => {
				...
				const systemResource = mockResource.mock.calls.find((call) =>
					call[0].includes("/system/"),
				);
			...
			it("should execute resource handler and return formatted result", async () => {
				...
				const uri = new URL(
					"f5xc://tenant/default/http_loadbalancer/example-lb",
				);
