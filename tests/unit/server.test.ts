...
import { createServer, F5XCApiServer, type ServerConfig } from "../../src/server.js";
import {
  clearF5XCEnvVars,
  isCI,
...
const { mockConnect, mockClose, mockTool, mockResource, mockPrompt } = vi.hoisted(() => ({
  ...
  describe("F5XCApiServer", () => {
    ...
    describe("resource registration", () => {
      ...
      it("should register namespace-scoped resources with correct URI template", async () => {
        ...
        const namespacedResource = mockResource.mock.calls.find((call) => call[0].includes("{namespace}"));
      ...
      it("should register system resources with correct URI template", async () => {
        ...
        const systemResource = mockResource.mock.calls.find((call) => call[0].includes("/system/"));
    ...
    describe("prompt registration", () => {
      ...
      it("should register enable_waf_protection prompt from upstream", async () => {
        ...
        const wafPrompt = mockPrompt.mock.calls.find((call) => call[0] === "enable_waf_protection");
