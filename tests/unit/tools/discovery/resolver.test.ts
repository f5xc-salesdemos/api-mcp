...
describe("Dependency Resolver", () => {
	...
	describe("resolveDependencies - Basic Resolution", () => {
		...
		it("should skip existing resources", () => {
			// Arrange
			vi.mocked(getResourceDependencies).mockImplementation(
				(domain, resource) => {
			...
			vi.mocked(getPrerequisiteResources).mockImplementation(
				(domain, resource) => {
					if (resource === "http-loadbalancer") {
						return [
							{
	...
	describe("resolveDependencies - Multi-Level Dependencies", () => {
		...
		it("should handle diamond dependencies (A → B, A → C, B → D, C → D)", () => {
				...
				(domain, resource) => {
					...
					if (resource === "resource-b") {
						return {
							...
							domain: "test",
							prerequisites: [
								{ domain: "test", resourceType: "resource-d", required: true },
							],
							optional: [],
						};
					}
			...
			const middleResources = [
				result.plan?.steps[1].resource,
				result.plan?.steps[2].resource,
			];
	...
	describe("resolveDependencies - Optional Dependencies", () => {
		it("should exclude optional dependencies by default", () => {
			...
			expect(result.plan?.totalSteps).toBe(2);
			expect(
				result.plan?.steps.some((s) => s.resource === "resource-required"),
			).toBe(true);
			expect(
				result.plan?.steps.some((s) => s.resource === "resource-optional"),
			).toBe(false);
		});
		...
		it("should include optional dependencies when includeOptional=true", () => {
			...
			expect(result.plan?.steps.some((s) => s.resource === "resource-a")).toBe(
				true,
			);
		});
	});
				...
				(domain, resource) => {
					const deps: Record<string, any> = {
						...
						"resource-d": {
							...
							domain: "test",
							prerequisites: [
								{ domain: "test", resourceType: "resource-e", required: true },
							],
							optional: [],
						},
						"resource-e": {
	...
	describe("resolveDependencies - oneOf Choices", () => {
		it("should include oneOf choices in workflow steps", () => {
			...
			expect(result.plan?.steps[0].oneOfChoices?.[0].field).toBe(
		...
		it("should expand alternatives when expandAlternatives=true", () => {
			...
			expect(result.plan?.alternatives[0].choiceField).toBe(
				"origin_pools_weights",
			);
		});
	});
	...
	describe("resolveDependencies - Subscription Requirements", () => {
		it("should include subscription requirements in plan", () => {
			...
			expect(result.plan?.subscriptions[0]).toContain(
				...
				(domain, resource) => {
					const deps: Record<string, any> = {
						...
						"resource-e": {
							...
							domain: "test",
							prerequisites: [
								{ domain: "test", resourceType: "resource-f", required: true },
							],
							optional: [],
						},
						"resource-f": {
							...
							domain: "test",
							prerequisites: [
								{ domain: "test", resourceType: "resource-g", required: true },
							],
							optional: [],
						},
						"resource-g": {
	...
	describe("formatCreationPlan", () => {
		it("should format plan with basic information", () => {
			...
			expect(formatted).toContain(
				"# Creation Plan for virtual/http-loadbalancer",
			);
			...
			expect(formatted).toContain(
				"### Step 2: create virtual/http-loadbalancer",
			);
			expect(formatted).toContain(
				"**Tool**: `f5xc-api-virtual-origin-pool-create`",
			);
		...
		it("should format plan with subscriptions", () => {
			...
			const plan: CreationPlan = {
				...
				alternatives: [],
				subscriptions: [
					"Web Application and API Protection (Advanced) - required",
				],
			};
		...
		it("should format plan with warnings", () => {
			...
			const plan: CreationPlan = {
				...
				warnings: [
					"No create tool found for dependency-resource",
			...
			expect(formatted).toContain(
				"⚠️ No create tool found for dependency-resource",
			);
		...
		it("should format plan with oneOf choices", () => {
			...
			expect(formatted).toContain(
				"origin_pool, pool_weights, ad_pool_priority",
			);
		});
				...
				alternatives: [
					{
						...
						steps: [],
						description:
							"Alternative using origin_pool for origin_pools_weights",
					},
				],
				subscriptions: [],
			};
