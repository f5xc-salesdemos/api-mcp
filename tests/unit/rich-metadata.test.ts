  ...
  describe("x-ves-example (Parameter Examples)", () => {
    ...
    it("should have tools with parameter examples from fixtures if available", () => {
      if (RICH_METADATA_SAMPLES.withParameters) {
        ...
        expect(tool).toBeDefined();
        expect(tool!.pathParameters.length).toBe(RICH_METADATA_SAMPLES.withParameters.pathParamCount);
        expect(tool!.queryParameters.length).toBe(RICH_METADATA_SAMPLES.withParameters.queryParamCount);
      }
    });
  });
