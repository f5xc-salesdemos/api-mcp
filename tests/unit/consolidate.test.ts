...
describe("consolidate", () => {
	...
	describe("getConsolidatedIndex", () => {
		...
		it("should identify full CRUD resources", () => {
			...
			expect(fullCrudResources.every((r) => r.operations.length >= 4)).toBe(
				true,
			);
		});
	...
	describe("getConsolidatedResource", () => {
		...
		it("should return undefined for non-existent resource", () => {
			const resource = getConsolidatedResource(
				"non-existent-resource-xyz-12345",
			);
	...
	describe("resolveConsolidatedTool", () => {
		...
		it("should return null for unavailable operation", () => {
			...
			const partialResource = index.resources.find(
				(r) => !r.operations.includes("update"),
			);

			if (partialResource) {
				const toolName = resolveConsolidatedTool(
					partialResource.name,
					"update",
				);
