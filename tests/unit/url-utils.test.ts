...
describe("URL Utilities", () => {
	describe("normalizePath", () => {
		it("should strip /api prefix from paths", () => {
			expect(normalizePath("/api/config/namespaces/default")).toBe(
		...
		it("should strip /api prefix from paths with resource names", () => {
			expect(
				normalizePath(
					"/api/config/namespaces/default/http_loadbalancers/my-lb",
				),
			).toBe("/config/namespaces/default/http_loadbalancers/my-lb");
		});

		it("should preserve paths without /api prefix", () => {
			expect(normalizePath("/config/namespaces/default")).toBe(
				"/config/namespaces/default",
			);
		});
		...
		it("should not modify paths that contain api elsewhere", () => {
			expect(normalizePath("/config/api-gateway/default")).toBe(
				"/config/api-gateway/default",
			);
		});
	...
	describe("normalizeF5XCUrl", () => {
		...
		describe("trailing slash handling", () => {
			it("should remove single trailing slash", () => {
				expect(
					normalizeF5XCUrl("https://tenant.console.ves.volterra.io/"),
				).toBe("https://tenant.console.ves.volterra.io");
			});
		...
		describe("port handling", () => {
			it("should preserve explicit ports", () => {
				expect(
					normalizeF5XCUrl("https://tenant.console.ves.volterra.io:8443"),
				).toBe("https://tenant.console.ves.volterra.io:8443");
			});
		});
	});
	...
	describe("extractTenantFromUrl", () => {
		it("should extract tenant from console URL", () => {
			expect(
				extractTenantFromUrl("https://f5-amer-ent.console.ves.volterra.io"),
			).toBe("f5-amer-ent");
		});

		it("should extract tenant from staging URL", () => {
			expect(
				extractTenantFromUrl("https://staging-test.staging.volterra.us"),
			).toBe("staging-test");
		});

		it("should extract tenant from protocol-less URL", () => {
			expect(extractTenantFromUrl("mytenant.console.ves.volterra.io")).toBe(
				"mytenant",
			);
		});
		...
		it("should handle hyphenated tenant names", () => {
			expect(
				extractTenantFromUrl("https://my-tenant-name.console.ves.volterra.io"),
			).toBe("my-tenant-name");
		});
	});
	...
	describe("verifyF5XCEndpoint", () => {
		...
		it("should return valid for 401 response (valid URL, needs auth)", async () => {
			...
			const result = await verifyF5XCEndpoint(
				"https://tenant.console.ves.volterra.io",
			);
		...
		it("should return invalid for network error with suggestions", async () => {
			...
			const result = await verifyF5XCEndpoint(
				"https://invalid.console.ves.volterra.io",
			);
	...
	describe("Integration: normalizePath with buildApiPath patterns", () => {
		...
		it.each(
			resourcePaths,
		)("should correctly normalize resource path: %s", (resourcePath: string) => {
