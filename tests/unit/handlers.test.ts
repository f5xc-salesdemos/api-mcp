	...
	describe("ResourceHandler", () => {
		...
		describe("readResource", () => {
			...
			describe("error handling", () => {
				...
				it("should throw error for unknown resource type", async () => {
					...
					await expect(
						handler.readResource(
							"f5xc://test-tenant/default/unknown-resource/example-resource",
						),
				...
				it("should throw F5XCApiError when buildApiPath returns null in execution mode", async () => {
					...
					const buildApiPathSpy = vi
						.spyOn(templates, "buildApiPath")
		...
		describe("listResourceTemplates", () => {
			...
			it("should include tenant in URI template", () => {
				...
				const templates = handler.listResourceTemplates();
				const namespaceScoped = templates.find((t) =>
					t.uriTemplate.includes("{namespace}"),
				);
			...
			it("should have namespace-scoped and system-scoped templates", () => {
				...
				const templates = handler.listResourceTemplates();
				const namespaceScoped = templates.filter((t) =>
					t.uriTemplate.includes("{namespace}"),
				);
				const systemScoped = templates.filter((t) =>
					t.uriTemplate.includes("/system/"),
				);
