...
| Tool | Description |
|------|-------------|
| `f5xc-api-virtual-get-security-config-create` | Get Security Config for HTTP Load Balancer. |
...
|--------|-------------|-------------|
| `all_http_loadbalancers` | All http loadbalancers |  |
| `http_loadbalancers_list` | Http loadbalancers list |  |
...
```bash
# Create resource
curl -X POST "https://${TENANT}.console.ves.volterra.io/api/config/namespaces/$${PARAM}/http_loadbalancers/get_security_config" \
