...
export function registerResolveDependenciesTool(server: McpServer): void {
  server.tool(
    ...
    DISCOVERY_TOOLS.resolveDependencies.description,
    {
      ...
      existingResources: z.array(z.string()).optional().describe("Resources that already exist (will be skipped)"),
      ...
      expandAlternatives: z.boolean().optional().default(false).describe("Include alternative paths for oneOf choices"),
    },
    async (args) => {
    ...
    async (args) => {
      ...
      if (args.toolNames && args.toolNames.length > 0) {
        ...
        return createTextResponse({
          ...
          totalTokens,
          estimates,
          formatted: args.detailed ? estimates.map((e) => formatCostEstimate(e)).join("\n\n---\n\n") : undefined,
        });
      }
