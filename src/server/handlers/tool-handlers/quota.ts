	...
	server.tool(
		...
		"Get current quota usage and limits for a namespace and resource type",
		{
			namespace: z
				.string()
				.describe(
					"Namespace to check quota for (e.g., 'default', 'production')",
				),
			resourceType: z
				.string()
				.optional()
				.describe(
					"Resource type to check (e.g., 'healthcheck', 'http-loadbalancer')",
				),
		},
		async (args) => {
			...
			try {
				...
				if (resourceType) {
					...
					const allQuotas = await quotaService.getAllNamespaceQuotas(
						namespace,
						httpClient,
					);
	...
	server.tool(
		...
		"List all resource quotas for a namespace with current usage",
		{
			...
			showOnlyLimited: z
				...
				.default(false)
				.describe(
					"Only show resources with quota limits (filter out unlimited)",
				),
		},
		async (args) => {
			...
			try {
				const httpClient = createHttpClient(credentialManager);
				const quotas = await quotaService.getAllNamespaceQuotas(
					namespace,
					httpClient,
				);
				...
				const filtered = showOnlyLimited
					? quotas.filter(
							(q) => q.limits.limit > 0 && q.limits.limit < Infinity,
						)
					: quotas;
	...
	server.tool(
		...
		"Clear the quota information cache to force fresh queries from the F5XC API",
		{
			namespace: z
				...
				.optional()
				.describe(
					"Specific namespace to clear (optional - clears all if omitted)",
				),
		},
		async (args) => {
