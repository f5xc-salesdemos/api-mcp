...
export function resolveNestedRefs(
	...
	if ("$ref" in obj && typeof obj["$ref"] === "string") {
		...
		if (resolved) {
			...
			const resolvedSchema = resolveNestedRefs(
				resolved,
				domain,
				depth + 1,
				new Set(visited),
	...
	for (const [key, value] of Object.entries(obj)) {
		if (key === "properties" && typeof value === "object" && value !== null) {
			...
			for (const [propKey, propValue] of Object.entries(
				...
				props[propKey] = resolveNestedRefs(
					propValue,
					domain,
					depth + 1,
					visited,
	...
	for (const [key, propSchema] of Object.entries(schema.properties)) {
		...
		if (hasDefault || hasRecommended) {
			defaults.push({
				...
				vesRequired:
					propSchema["x-ves-required"] === true ||
					propSchema["x-ves-required"] === "true",
		...
		if (propSchema.items && typeof propSchema.items === "object") {
			defaults.push(
				...extractFieldDefaults(
					propSchema.items as ResolvedSchema,
					`${fieldPath}[]`,
				),
			);
		}
 ...
 */
export function getMinimumConfigurationFromSchema(
	toolName: string,
	...
	if (schema.oneOf && Array.isArray(schema.oneOf)) {
		groups.push({
			...
			options: schema.oneOf.map((opt, idx) => ({
				fieldName:
					typeof opt.title === "string" ? opt.title : `option${idx + 1}`,
				description:
