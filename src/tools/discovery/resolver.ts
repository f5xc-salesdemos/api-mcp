...
export function resolveDependencies(params: ResolveParams): ResolveResult {
	...
	for (const dep of sortedDeps) {
		...
		const dependsOn = prereqs
			.filter(
				(p) => !existingSet.has(createResourceKey(p.domain, p.resourceType)),
			)
	...
	if (targetToolName) {
		...
		warnings.push(
			`No create tool found for target resource ${domain}/${resource}.`,
		);
	}
	...
	const plan: CreationPlan = {
		...
		subscriptions,
		existingResources:
			existingResources.length > 0 ? existingResources : undefined,
		complexity,
	};
...
export function formatCreationPlan(plan: CreationPlan): string {
	...
	for (const step of plan.steps) {
		lines.push(
			`### Step ${step.stepNumber}: ${step.action} ${step.domain}/${step.resource}`,
		);
		...
		if (step.oneOfChoices && step.oneOfChoices.length > 0) {
			...
			for (const choice of step.oneOfChoices) {
				lines.push(
					`  - \`${choice.field}\`: Choose one of [${choice.options.join(", ")}]`,
				);
			}
		}
