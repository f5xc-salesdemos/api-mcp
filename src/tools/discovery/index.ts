		...
		inputSchema: {
			...
			properties: {
				...
				operations: {
					...
					items: { type: "string" },
					description:
						"Filter by operation type(s): create, get, list, update, delete",
				},
				excludeDangerous: {
					type: "boolean",
		...
		inputSchema: {
			...
			properties: {
				...
				pathParams: {
					type: "object",
					description:
						"Path parameters (e.g., { namespace: 'default', name: 'example-resource' })",
		...
		inputSchema: {
			...
			properties: {
				resource: {
					type: "string",
					description:
						"The resource name (e.g., 'http-loadbalancer', 'origin-pool')",
				},
				domain: {
					type: "string",
					description:
						"The domain containing the resource (e.g., 'virtual', 'network', 'api')",
				},
				action: {
					type: "string",
					enum: [
		...
		inputSchema: {
			...
			properties: {
				resource: {
					type: "string",
					description:
						"The target resource to create (e.g., 'http-loadbalancer', 'origin-pool')",
				},
				domain: {
					type: "string",
					description:
						"The domain containing the resource (e.g., 'virtual', 'network')",
				},
				existingResources: {
					type: "array",
					items: { type: "string" },
				...
				includeOptional: {
					type: "boolean",
					description:
						"Include optional dependencies in the plan (default: false)",
					default: false,
				},
				maxDepth: {
					type: "number",
				...
				expandAlternatives: {
					type: "boolean",
					description:
						"Include alternative paths for oneOf choices (default: false)",
					default: false,
				},
			},
			required: ["resource", "domain"],
		...
		inputSchema: {
			...
			properties: {
				...
				detailed: {
					type: "boolean",
					description:
						"Include detailed breakdown of token usage and latency (default: true)",
					default: true,
				},
			},
		},
		...
		inputSchema: {
			...
			properties: {
				toolName: {
					type: "string",
					description:
						"The exact tool name (e.g., 'f5xc-api-virtual-http-loadbalancer-create')",
				},
			},
			required: ["toolName"],
		},
