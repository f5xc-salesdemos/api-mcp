 ...
 */
export function estimateMultipleToolsCost(
	toolNames: string[],
): ToolCostEstimate[] {
...
export function estimateWorkflowCost(plan: CreationPlan): WorkflowCostEstimate {
	...
	const steps = plan.steps.map((step: WorkflowStep) => {
		...
		if (!toolExists(step.toolName)) {
			warnings.push(
				`Tool '${step.toolName}' not found - estimates may be inaccurate`,
			);
		}
 ...
 */
export function formatWorkflowCostEstimate(
	...
	lines.push(
		`- **Estimated Total Time**: ~${(estimate.estimatedTotalMs / 1000).toFixed(1)}s`,
	);
	lines.push("");
