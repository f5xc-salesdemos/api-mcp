...
export function suggestParameters(toolName: string): SuggestionResult | null {
	...
	if (minConfig?.example_json) {
		try {
			const payload = JSON.parse(minConfig.example_json) as Record<
				string,
				unknown
			>;
			...
			return {
				...
				description:
					minConfig.description ||
					`Example payload for ${tool.resource} ${tool.operation}`,
				source: "spec",
...
function buildNotesFromMinConfig(minConfig: MinimumConfiguration): string[] {
	...
	if (
		minConfig.mutually_exclusive_groups &&
		minConfig.mutually_exclusive_groups.length > 0
	) {
		for (const group of minConfig.mutually_exclusive_groups) {
			notes.push(
				`Mutually exclusive: ${group.fields.join(" OR ")} - ${group.reason}`,
			);
		}
	}
