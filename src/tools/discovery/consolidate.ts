...
export function getConsolidatedIndex(): ConsolidatedIndex {
	...
	for (const [, opMap] of resourceMap) {
		...
		const toolMap: Record<CrudOperation, string> = {} as Record<
			CrudOperation,
			string
		>;
 ...
 */
export function getConsolidatedResource(
	name: string,
): ConsolidatedResource | undefined {
 ...
 */
export function getConsolidatedByDomain(
	domain: string,
...
export function searchConsolidatedResources(
	...
	for (const resource of index.resources) {
		...
		if (domains && domains.length > 0) {
			if (
				!domains.some((d) => d.toLowerCase() === resource.domain.toLowerCase())
			) {
				continue;
			}
		}
