 ...
 */
function getDomainDisplayInfo(domain: string): {
	displayName: string;
	description: string;
} {
	...
	for (const resource of resources) {
		...
		for (const err of resourceErrors) {
			...
			errors.push({
				...
				resolution: [
					err.resolution,
					...(httpError?.prevention?.slice(0, 2) || []),
				],
			});
		}
	}
		...
		if (!errors.some((e) => e.statusCode === code)) {
			...
			if (httpError) {
				errors.push({
					...
					resolution: httpError.diagnosticSteps
						.slice(0, 3)
						.map((s) => s.description),
				});
			}
...
function analyzeDangerLevels(domain: string): DangerAnalysis {
	...
	if (total > 0) {
		analysis.safePercentage = Math.round(
			((analysis.low + analysis.medium) / total) * 100,
		);
	}
 ...
 */
function convertUpstreamWorkflow(
	upstream: GuidedWorkflow,
): RecommendedWorkflow {
	return {
		name: upstream.name,
...
function generateWorkflows(domain: string): RecommendedWorkflow[] {
	...
	const workflows: RecommendedWorkflow[] = upstreamWorkflows.map(
		convertUpstreamWorkflow,
	);
 ...
 */
export function getDomainBestPractices(
	domain: string,
): DomainBestPractices | null {
	const index = getToolIndex();
	const domainTools = index.tools.filter((t) => t.domain === domain);
 ...
 */
export function queryBestPractices(
...
export function getAllDomainsSummary(): Array<{
	...
	const domains = new Map<
		string,
		{ count: number; safe: number; dangerous: number }
	>();

	for (const tool of index.tools) {
		const existing = domains.get(tool.domain) ?? {
			count: 0,
			safe: 0,
			dangerous: 0,
		};
...
export function formatBestPractices(practices: DomainBestPractices): string {
	...
	if (practices.workflows.length > 0) {
		...
		for (const workflow of practices.workflows) {
			...
			lines.push(
				`*${workflow.description}* (Complexity: ${workflow.complexity})`,
			);
	...
	if (
		practices.troubleshootingGuides &&
		practices.troubleshootingGuides.length > 0
	) {
