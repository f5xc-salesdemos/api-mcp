...
export function searchTools(
	...
	if (operations && operations.length > 0) {
		...
		if (candidateToolIds) {
			...
			candidateToolIds = new Set(
				[...candidateToolIds].filter((id) => opFilteredIds.has(id)),
			);
		} else {
	...
	for (const [toolId, baseScore] of indexScores) {
		...
		if (
			normalizeText(tool.domain).includes(
				normalizeText(query.split(" ")[0] || ""),
			)
		) {
			score *= 1.2;
		}
		...
		if (score >= minScore) {
			...
			if (includeDependencies && tool.operation === "create") {
				...
				const resourceNames = prereqs.map(
					(p) => `${p.domain}/${p.resourceType}`,
				);
