...
export function buildDependencyGraph(
	...
	for (const [resourceKey, operations] of resourceMap) {
		const { domain, resource } = parseResourceKey(
			resourceKey as `${string}/${string}`,
		);
	...
	if (opts.computeCreationOrder) {
		for (const resourceKey of Object.keys(dependencies)) {
			...
			if (dep) {
				dep.creationOrder = computeCreationOrder(
					resourceKey,
					dependencies,
					opts.maxDepth,
				);
...
function buildReverseDependencies(
	...
	for (const deps of Object.values(dependencies)) {
		for (const ref of deps.requires) {
			const refKey = createResourceKey(
				ref.domain || "unknown",
				ref.resourceType,
			);
...
export function getPrerequisites(
	...
	return deps.requires.map((ref) =>
		createResourceKey(ref.domain || "unknown", ref.resourceType),
	);
}
...
export function getDependents(
	...
	return reverseDeps.map((ref) =>
		createResourceKey(ref.domain, ref.resourceType),
	);
}
 ...
 */
export function getResourcesBySubscription(
	addonService: string,
	graph: DependencyGraph,
): string[] {
