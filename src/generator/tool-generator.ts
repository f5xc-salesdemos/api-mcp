...
function buildDocumentationResponse(operation: ParsedOperation): DocumentationResponse {
  ...
  const prerequisites = operation.operationMetadata?.conditions?.prerequisites ?? generatePrerequisites(operation);
...
export function registerTool(
  ...
  server.tool(operation.toolName, description, schema.shape, async (params: Record<string, unknown>) => {
    try {
