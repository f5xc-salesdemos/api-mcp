	...
	for (const [paramName, paramRules] of Object.entries(rules)) {
		...
		if (paramRules["ves.io.schema.rules.repeated.max_items"]) {
			const maxItems = parseInt(
				paramRules["ves.io.schema.rules.repeated.max_items"],
			);
...
function buildDocumentationResponse(
	...
	for (const param of operation.pathParameters) {
		parameters.push({
			...
			location: "path",
			type:
				((param.schema as Record<string, unknown>)?.type as string) ?? "string",
	...
	const prerequisites =
		operation.operationMetadata?.conditions?.prerequisites ??
		generatePrerequisites(operation);
		...
		return {
			metadata: {
				name:
					...
					`example-${resource}`,
				namespace:
					examples["namespace"] ?? examples["metadata.namespace"] ?? "default",
			},
			spec: {},
		};
	}
