...
function extractDomainOperations(
	...
	const curlMap =
		curlExamples ??
		extractCurlExamplesFromSchemas(
	...
	for (const [path, pathItem] of sortedPaths) {
		...
		for (const method of httpMethods) {
			...
			const confirmationRequired =
				operation["x-ves-confirmation-required"] ?? false;
			...
			const discoveredResponseTimeMs =
				operation["x-discovered-response-time-ms"];
			const discoveredSampleSize = operation["x-discovered-sample-size"];
			const discoveryMetadata: DiscoveryMetadata | undefined =
				discoveredResponseTimeMs !== undefined ||
				discoveredSampleSize !== undefined
					? {
			...
			if (
				pathItemAny["x-displayname"] &&
				typeof pathItemAny["x-displayname"] === "string"
			) {
			...
			const componentSchemas =
				(spec.components?.schemas as Record<string, unknown>) ?? {};
			const extractedDeps = extractOperationDependencies(
				requestBodySchema,
				componentSchemas,
			);

			// Resolve domain for each reference
			const dependencies: ResourceReference[] = extractedDeps.references.map(
				(ref) => ({
					...ref,
			...
			const subscriptionRequirements: SubscriptionRequirement[] =
