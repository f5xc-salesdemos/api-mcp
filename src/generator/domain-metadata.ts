 ...
 */
function transformCliMetadata(
	raw: RawSpecEntry["x-f5xc-cli-metadata"],
...
function transformSpecEntry(raw: RawSpecEntry): DomainMetadata {
	...
	return {
		...
		primaryResources: (raw["x-f5xc-primary-resources"] || []).map(
...
export function loadSpecIndex(): SpecIndex {
	...
	if (!existsSync(indexPath)) {
		throw new Error(
			`Spec index not found at ${indexPath}. Run 'npm run sync-specs' first.`,
		);
	}
...
export function getAllDomainCategories(): string[] {
	...
	const categories = new Set(
		index.specifications.map((spec) => spec.domainCategory),
	);
...
export function getAllUiCategories(): string[] {
	const index = loadSpecIndex();
	const categories = new Set(
		index.specifications.map((spec) => spec.uiCategory),
	);
	return Array.from(categories).sort();
}
 ...
 */
function transformGuidedWorkflowStep(
	raw: RawGuidedWorkflowStep,
): GuidedWorkflowStep {
	return {
		order: raw.order,
		action: raw.action,
 ...
 */
export function getHttpErrorResolution(
	statusCode: number,
): HttpErrorResolution | undefined {
 ...
 */
export function getResourceErrorPatterns(
	resourceType: string,
...
export function isCriticalResource(resourceType: string): boolean {
	...
	return critical.some(
		(r) => r.toLowerCase().replace(/-/g, "_") === normalized,
	);
}
...
export function getAcronymByName(name: string): AcronymEntry | undefined {
	const index = loadAcronyms();
	return index.acronyms.find(
		(a) => a.acronym.toLowerCase() === name.toLowerCase(),
	);
}
