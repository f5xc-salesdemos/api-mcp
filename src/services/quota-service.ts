...
export class QuotaService {
	...
	constructor() {
		...
		this.cacheTTL =
			parseInt(
				process.env.F5XC_QUOTA_CACHE_TTL || String(DEFAULT_CACHE_TTL / 1000),
				10,
			) * 1000;
	}
	...
	async getQuotaStatus(
		...
		logger.debug(
			`Quota cache miss for ${resourceType} in ${namespace} - fetching from API`,
		);
		...
		const resourceUsage = usageData.find(
			(u) => u.resourceType === f5xcResourceType,
		);
		...
		const quotaInfo = this.calculateQuotaInfo(
